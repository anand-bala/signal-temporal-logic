...
# This function will add lcov evaluation for target <TNAME>. Only sources of this target
# will be evaluated and no dependencies will be added. It will call geninfo on any
# source file of <TNAME> once and store the info file in the same directory.
#
# Note: This function is only a wrapper to define this function always, even if coverage
# is not supported by the compiler or disabled. This function must be defined here,
# because the module will be exited, if there is no coverage support by the compiler or
...
# This function will merge lcov files to a single target file. Additional lcov flags may
# be set with setting LCOV_EXTRA_FLAGS before calling this function.
function(lcov_merge_files OUTFILE ...)
  ...
  add_custom_command(
    ...
    COMMAND ${LCOV_BIN} --quiet -r ${OUTFILE} ${LCOV_REMOVE_PATTERNS} --output-file
...
# Add a new global target to generate initial coverage reports for all targets. This
# target will be used to generate the global initial info file, which is used to gather
# even empty report data.
...
# This function will add initial capture of coverage data for target <TNAME>, which is
# needed to get also data for objects, which were not loaded at execution time. It will
# call geninfo for every source file of <TNAME> once and store the info file in the same
# directory.
function(lcov_capture_initial_tgt TNAME)
  # We don't have to check, if the target has support for coverage, thus this will be
  # checked by add_coverage_target in Findcoverage.cmake. Instead we have to determine
  # which gcov binary to use.
  ...
  foreach(FILE ${SOURCES})
    ...
    add_custom_command(
      OUTPUT ${OUTFILE}
      COMMAND
        ${GCOV_ENV} ${GENINFO_BIN} --quiet --base-directory ${PROJECT_SOURCE_DIR}
        --initial --gcov-tool ${GCOV_BIN} --output-filename ${OUTFILE}
...
# This function will generate the global info file for all targets. It has to be called
# after all other CMake functions in the root CMakeLists.txt file, to get a full list of
# all targets that generate coverage data.
function(lcov_capture_initial)
  # Skip this function (and do not create the following targets), if there are no input
  # files.
...
# Add a new global target to generate coverage reports for all targets. This target will
# be used to generate the global info file.
...
# This function will add capture of coverage data for target <TNAME>, which is needed to
# get also data for objects, which were not loaded at execution time. It will call
# geninfo for every source file of <TNAME> once and store the info file in the same
# directory.
function(lcov_capture_tgt TNAME)
  ...
  set(GENINFO_FILES "")
  foreach(FILE ${SOURCES})
    # Generate coverage files. If no .gcda file was generated during execution, the
    # empty coverage file will be used instead.
    ...
    add_custom_command(
      OUTPUT ${OUTFILE}
      COMMAND
        test -f "${TDIR}/${FILE}.gcda" && ${GCOV_ENV} ${GENINFO_BIN} --quiet
        --base-directory ${PROJECT_SOURCE_DIR} --gcov-tool ${GCOV_BIN} --output-filename
        ${OUTFILE} ${GENINFO_EXTERN_FLAG} ${TDIR}/${FILE}.gcda || cp ${OUTFILE}.init
        ${OUTFILE}
  ...
  add_custom_target(
    ${TNAME}-genhtml
    COMMAND
      ${GENHTML_BIN} --quiet --sort --prefix ${PROJECT_SOURCE_DIR} --baseline-file
      ${LCOV_DATA_PATH_INIT}/${TNAME}.info --output-directory ${LCOV_HTML_PATH}/${TNAME}
      --title "${CMAKE_PROJECT_NAME} - target ${TNAME}" ${GENHTML_CPPFILT_FLAG}
      ${OUTFILE}
...
function(lcov_capture)
  ...
  # Add a new global target for all lcov targets. This target could be used to generate
  # the lcov html output for the whole project instead of calling <TARGET>-geninfo and
  # <TARGET>-genhtml for each target. It will also be used to generate a html site for
  # all project data together instead of one for each target.
  if(NOT TARGET lcov)
...
# Add a new global target to generate the lcov html report for the whole project instead
# of calling <TARGET>-genhtml for each target (to create an own report for each target).
# Instead of the lcov target it does not require geninfo for all targets, so you have to
# call <TARGET>-geninfo to generate the info files the targets you'd like to have in
# your report or lcov-geninfo for generating info files for all targets before calling
# lcov-genhtml.
...
if(NOT TARGET lcov-genhtml)
  add_custom_target(
    lcov-genhtml
    COMMAND
      ${GENHTML_BIN} --quiet --output-directory ${LCOV_HTML_PATH}/selected_targets
      --title \"${CMAKE_PROJECT_NAME} - targets `find ${LCOV_DATA_PATH_CAPTURE} -name
      \"*.info\" ! -name \"all_targets.info\" -exec basename {} .info \\\;`\" --prefix
